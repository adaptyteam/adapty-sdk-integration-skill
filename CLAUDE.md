...
- `feedback-endpoint/` — the endpoint that receives the end-of-run feedback payload (from this skill and from the Adapty CLI) and forwards it to Slack + Airtable. One implementation, two deployments: a Cloudflare Worker (`src/index.js`, deployed by `.github/workflows/feedback-endpoint-deploy.yml` on merge to main) and the legacy Vercel function (`api/sdk-integration-feedback.js`), kept alive because its URL is baked into already-published skill versions and able to relay to the Worker via `FORWARD_URL`. The body is clamped before delivery (`src/validate.js`) — the endpoint is public and unauthenticatable, and `slack_text` reaches a Slack channel verbatim. Its README documents the env vars, the cutover, the validation rules, and the rule that an Airtable column must exist before code that sends it. Has its own tests: `node --test test/endpoint.test.js` from inside that directory.
