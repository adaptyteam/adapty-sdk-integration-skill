...
1. **Paywall approach** — which do they want?
   - **Paywall Builder** (recommended): Adapty renders paywalls in a no-code visual editor; no paywall UI to build
     - **iOS and Android:** Present this option as **Flow Builder** instead. Flow Builder is the v4 successor to Paywall Builder and also supports onboarding flows. The `paywallApproach` state value for this choice on iOS and Android is `flow_builder`. Note: Flow Builder requires the iOS or Android SDK **v4+**; see Stage 1 in `references/ios.md` or `references/android.md`.
