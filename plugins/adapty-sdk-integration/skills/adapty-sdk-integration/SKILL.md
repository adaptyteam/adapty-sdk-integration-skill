...
1. **Paywall approach** — which do they want?
   - **Paywall Builder** (recommended): Adapty renders paywalls in a no-code visual editor; no paywall UI to build
     - **iOS and React Native:** Present this option as **Flow Builder** instead. Flow Builder is the v4 successor to Paywall Builder and also supports onboarding flows. The `paywallApproach` state value for this choice on iOS and React Native is `flow_builder`. Note: Flow Builder requires the iOS or React Native SDK **v4+**, which is currently a **pre-release (beta)** — at install time the exact version must be pinned (`4.0.0-beta.1`), and React Native v4 also requires RN 0.75+ and Swift Package Manager (dynamic frameworks); see Stage 1 in `references/ios.md` or `references/react-native.md`.
...
**State update:** Set `paywallApproach` to `paywall_builder` (or `flow_builder` on iOS or React Native), `custom`, or `observer`. Set `integrations` to the array of selected integration keys (e.g. `["amplitude", "appsflyer"]`), or `[]` if none. Set `appPreference` to `existing` or `new`. Set `phasesCompleted = 2`.
...
#### `paywallApproach == "flow_builder"` (Flow Builder, iOS or React Native) — dashboard path
...
2. **Create the placement** at [Adapty Dashboard → Placements](https://app.adapty.io/placements):
   ...
   - Set a **Developer ID** (e.g. `main`, `onboarding`, `settings`) — this is the exact string the SDK uses in `getFlow`
